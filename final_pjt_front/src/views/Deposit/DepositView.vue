<template>
  ...
  const sortDescending = Object.fromEntries(months.map(month => [month, Object.fromEntries(preferred.map(p => [p, false]))]));
  ...
  onMounted(async () => {
    try {
      ...
      deposits.value = counterStore.DepositInfos.map(deposit => ({
        ...deposit,
        options: Object.fromEntries(deposit.options.map(option => [option.save_trm, option])),
      }));
    } catch (error) {
      console.log(error);
    }
  ...
  const highlightRow = (index) => {
    ...
    console.log(index)
    console.log(document.querySelectorAll('.container .table tbody tr')[index])
  };
